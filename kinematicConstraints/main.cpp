...
std::istream& operator>>(std::istream& in, CreateSTP::OptimizationObjective& objective)
{
    std::string token;
    in >> token;
    if (token == "MinMakeSpan")
        objective = CreateSTP::MinMakeSpan;
    else if (token == "MaxThroughput")
        objective = CreateSTP::MaxThroughput;
    else if (token == "Hybrid")
        objective = CreateSTP::Hybrid;
...
int main(int argc, char** argv) {
  ...
  desc.add_options()
      ...
      ("objective", po::value<CreateSTP::OptimizationObjective>(&objective)->default_value(CreateSTP::MinMakeSpan)->multitoken(), "optimization objective")
  ...
  {
    ScopedTimer timer;
    CreateSTP stp(plan, delta, objective, disableVminMaximization, outputFile);
  }
