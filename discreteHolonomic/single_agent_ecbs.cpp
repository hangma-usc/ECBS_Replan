...
SingleAgentECBS::SingleAgentECBS(int start_location, int goal_location, MapLoader::orientation_t start_orientation,
                  const double* my_heuristic, const bool* my_map, int map_size, const int* moves_offset,
                  const int* actions_offset, const EgraphReader* egr, double e_weight, bool tweak_g_val) :
    my_heuristic(my_heuristic), my_map(my_map), moves_offset(moves_offset), actions_offset(actions_offset), egr(egr) {
...
bool SingleAgentECBS::findPath(double f_weight, const vector < list< pair<int, int> > >* constraints, bool* res_table, size_t max_plan_len) {
  ...
  while ( !focal_list.empty() ) {
    ...
    int next_id;
    MapLoader::orientation_t next_orientation;
